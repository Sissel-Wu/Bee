...
{
    using ExamplePair = Tuple<XmlPath, XmlPath>;
    
    public class HDTWitnessFunctions: DomainLearningLogic
    {
        public static IEnumerable<IMapper<XmlNode>> Mappers;
        public static IEnumerable<IFeature<XmlNode, XmlPath>> Features;
        public static IdMap<XmlNode, XmlPath> IdMap;
            ...
            LearningTask<ExampleSpec> task, CancellationToken cancelToken)
        {
            ...
            var outputs = examples[inputState] as ISet<ExamplePair>;
            Debug.Assert(outputs != null, nameof(outputs) + " != null");

            if (!outputs.Any()) // base case, should be synthesized by "EmptyProgram()"
            ...
            foreach (var partition in CommonDeductions<XmlNode, XmlPath>.EnumeratePartitions(outputs, IdMap))
            {
                var complete = true;
        ...
        public ExampleSpec WitnessLetTerm(GrammarRule rule, ExampleSpec spec)
        {
            ...
            var term = CommonDeductions<XmlNode, XmlPath>.Unify(outputs.ToHashSet(), IdMap);
            if (term == null)
                return null;
        ...
        public ExampleSpec WitnessMapTransform_FilteredPaths(GrammarRule rule, ExampleSpec spec)
        {
            ...
            var inputState = spec.ProvidedInputs[0];
            var allInPaths = inputState[rule.Grammar.InputSymbol].ToEnumerable().Cast<XmlPath>();
            var outputs = spec.Examples[inputState].ToEnumerable().Cast<ExamplePair>().ToList();
        ...
        public ExampleSpec WitnessFilterPaths_Pred(GrammarRule rule, ExampleSpec spec)
        {
            ...
            var inputState = spec.ProvidedInputs[0];
            var allInPaths = inputState[rule.Grammar.InputSymbol].ToEnumerable().Cast<XmlPath>().ToList();
            var outputs = spec.Examples[inputState].ToEnumerable().Cast<XmlPath>().ToList();
            var labels = allInPaths.Select(path => outputs.Contains(path)).ToList();
            var decisionTree = new DecisionTree<XmlNode, XmlPath>(allInPaths, Features.ToList(), labels);
            decisionTree.Learn();
